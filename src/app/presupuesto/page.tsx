      ...
      <main className="bg-paper pt-[var(--header-h)]">
        <div className="border-b border-line bg-white">
          <div className="container-x py-10 lg:py-14">
            ...
            <h1 className="mt-5 max-w-2xl text-3xl font-semibold tracking-[-0.02em] text-navy-900 sm:text-[2.7rem] sm:leading-[1.1]">
              Crea tu presupuesto personalizado
            </h1>
            <p className="mt-4 max-w-xl text-[17px] leading-relaxed text-navy-600">
            ...
            <ul className="mt-7 grid gap-3 sm:grid-cols-3">
              {HIGHLIGHTS.map((h) => (
                <li key={h.title} className="flex items-start gap-3 border-t border-line-strong pt-4">
                  <Icon name={h.icon} size={16} className="mt-0.5 shrink-0 text-brand-600" />
                  <span>
                    <span className="block text-sm font-semibold text-navy-900">{h.title}</span>
                    <span className="block text-[13px] text-navy-400">{h.text}</span>
                  </span>
                </li>
              ))}
            </ul>
        ...
        <div className="container-x max-w-4xl py-10 lg:py-14">
          <div id="wizard-top" className="scroll-mt-24 rounded-[8px] border border-line bg-white p-6 sm:p-9">
            <QuoteWizard />
          </div>
        </div>
      </main>
